...
import styles from '../postNewJob/post-new-job.module.scss';
...
const EditJob = (props: postNewJobPopup) => {
	...
	const [salary, setSalary] = useState(props.children.salary)
	const [companyName, setcompanyName] = useState(props.children.company_name)
	const [zone, setzone] = useState(props.children.zone)
	const [proffesion, setproffesion] = useState(props.children.profession)
	const [region, setregion] = useState(props.children.region)
	const [manner, setManner] = useState(props.children.manner)
	const [level, setLevel] = useState(props.children.experienced_level)
	const [scope, setScope] = useState(props.children.scope)
	...
	const post = async () => {
		...
		if (description.length == 0 || title.length == 0) {
			...
			const res = await jobService.editJob({
				title: title,
				description: description,

				id: props.children._id,
				salary: salary,
